...
#define UH_RMTWEB_UNAME_MAXLEN	64
#define	MD5_LEN	32
...
struct rmt_web_ctl{
	char username[UH_RMTWEB_UNAME_MAXLEN];
	char md5_of_key[MD5_LEN];
	int uname_len;
	int passwd_len;
	int isenable;
};
...
struct listener {
	...
	/*added at 20140320********************************************************/
	short tlv_expt_len;//预期读出的TLV内容长度
	short tlv_type;
	int tlv_sid;	//当前接收的TLV片段所属的id,用以找到对应的client
	int tlv_buf[UH_LIMIT_MSGHEAD];
	int bf_stpos;
	unsigned int status;
	struct client *tlvcl;
	/*added at 20140320********************************************************/
...
struct client {
	...
	struct client *next;
	/*added at 20140320********************************************************/
	unsigned short tlv_sid;	//socket 复用,服务器发送的标志码
